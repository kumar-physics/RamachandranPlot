...
def read_csv(fname):
    ...
    with open(fname) as csvfile:
        ...
        for row in spamreader:
            if row[4] not in csv_data:
                csv_data[row[4]]={}
            if row[1] in amino_acid_dict:
                if row[1] not in csv_data[row[4]]:
                    csv_data[row[4]][row[1]] = {'phi': [], 'psi': []}
                csv_data[row[4]][row[1]]['phi'].append(float(row[2]))
                csv_data[row[4]][row[1]]['psi'].append(float(row[3]))
        for exp in csv_data:
            for amino_acid in csv_data[exp]:
                csv_data[exp][amino_acid]['phi'] = numpy.array(csv_data[exp][amino_acid]['phi'])
                csv_data[exp][amino_acid]['psi'] = numpy.array(csv_data[exp][amino_acid]['psi'])
    return csv_data
...
def calculate_density2(pdb_file, out_dir):
    ...
    lab_y = s
    for exp in pdb_data:
        if exp not in ['X-RAY DIFFRACTION','SOLUTION NMR','ELECTRON CRYSTALLOGRAPHY','NEUTRON DIFFRACTION','SOLID-STATE NMR']:
            out_dir = '{}/{}'.format(out_dir,exp)
            for res in aa:
                out_path = '{}/{}'.format(out_dir, res)
                if not os.path.exists(out_path):
                    os.makedirs(out_path)
                out_base_file_name = '{}/{}_{}'.format(out_path, exp,res)
                try:
                    pdb_density = cal_hist(pdb_data[exp][res]['phi'], pdb_data[exp][res]['psi'], s, ds).T
                    ...
                    pdb_data_points = len(pdb_data[exp][res]['psi'])
                ...
                for res in aa:
                    if res != 'GLY':  # Its better to exclude glycine
                        try:
                            pdb_all['phi'] = numpy.append(pdb_all['phi'], pdb_data[exp][res]['phi'])
                            pdb_all['psi'] = numpy.append(pdb_all['psi'], pdb_data[exp][res]['psi'])
                        except KeyError:
                            pass
...
if __name__ == "__main__":
    pdb_file = sys.argv[1]
    out_dir = sys.argv[2]
    calculate_density2(pdb_file, out_dir)
    # calculate_density2( '/Users/kumaran/af_dihearal/pdb/pdb_test.csv',
    #                   '/Users/kumaran/af_dihearal/output4')
